...
http_client_handle_t http_client_init(const http_client_config_t *config)
{
    ...
    _success = ((client = calloc(1, sizeof(http_client_t))) &&
    ...
    _success = ((client->transport_list = transport_list_init()) &&
    ...
    _success = ((ssl = transport_ssl_init()) &&
    ...
    _success = ((client->request->buffer->data = malloc(client->buffer_size)) &&
    ...
    {
        _success = ((http_client_set_header(client, "User-Agent", DEFAULT_HTTP_USER_AGENT) == WEB_OK) &&
    ...
    {
        _success = ((http_client_set_url(client, config->url) == WEB_OK) &&
...
web_err_t http_client_set_url(http_client_handle_t client, const char *url)
{
    ...
    if (old_path && client->connection_info.path && strcasecmp(old_path, (const void *)client->connection_info.path) != 0)
    {
...
web_err_t http_client_myperform(http_client_handle_t client, my_post_data_t *post_data)
{
    web_err_t err;
    do
    {
